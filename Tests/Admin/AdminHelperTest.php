...
class AdminHelperTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGetElementAccessPath()
    {
        ...
        $sub_object = $this->getMock('stdClass', array('getAnd'));
        $sub2_object = $this->getMock('stdClass', array('getMore'));

        $object->expects($this->atLeastOnce())->method('getPathToObject')->will($this->returnValue(array($sub_object)));
        $sub_object->expects($this->atLeastOnce())->method('getAnd')->will($this->returnValue($sub2_object));
        $sub2_object->expects($this->atLeastOnce())->method('getMore')->will($this->returnValue('Value'));
    ...
    public function testAppendFormFieldElementNested()
    {
        ...
        $sub_object = $this->getMock('stdClass', array('getAnd'));
        $sub2_object = $this->getMock('stdClass', array('getMore'));
        $sub3_object = $this->getMock('stdClass', array('getFinalData'));
        ...
        $childFormBuilder = new FormBuilder('subObject', get_class($sub_object), $eventDispatcher, $formFactory);

        $object->expects($this->atLeastOnce())->method('getSubObject')->will($this->returnValue(array($sub_object)));
        $sub_object->expects($this->atLeastOnce())->method('getAnd')->will($this->returnValue($sub2_object));
        $sub2_object->expects($this->atLeastOnce())->method('getMore')->will($this->returnValue(array($sub3_object)));
        $sub3_object->expects($this->atLeastOnce())->method('getFinalData')->will($this->returnValue('value'));
